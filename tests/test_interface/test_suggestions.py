...
class TestDataclasses:
    def test_suggestion_fields(self) -> None:
        s = Suggestion(text="hello", source="related", score=0.8, collection="ml-ai")
        assert s.text == "hello"
        assert s.source == "related"
        assert s.score == 0.8
        assert s.collection == "ml-ai"
    ...
    def test_suggestion_result_with_data(self) -> None:
        s = Suggestion(text="foo", source="gap", score=0.2, collection="quant-trading")
...
class TestFindRelated:
    ...
    def test_returns_suggestions_sorted_by_score(self, mock_vs: MagicMock) -> None:
        ...
        results = self.engine._find_related(DUMMY_VECTOR, ["ml-ai"])

        assert len(results) == 2
        assert results[0].score == 0.9
        assert results[0].source == "related"
        assert results[0].collection == "ml-ai"
    ...
    def test_aggregates_across_collections(self, mock_vs: MagicMock) -> None:
        ...
        results = self.engine._find_related(DUMMY_VECTOR, ["ml-ai", "quant-trading"])
...
class TestFindGaps:
    ...
    def test_gap_when_low_score(self, mock_emb_cls: MagicMock, mock_vs: MagicMock) -> None:
        ...
        gaps = self.engine._find_gaps(
            "machine learning and neural networks", DUMMY_VECTOR, ["ml-ai"]
        )
        assert len(gaps) >= 1
    ...
    def test_empty_kb_all_gaps(self, mock_emb_cls: MagicMock, mock_vs: MagicMock) -> None:
        ...
        gaps = self.engine._find_gaps(
            "transformers and attention mechanisms", DUMMY_VECTOR, ["ml-ai"]
        )
        assert len(gaps) >= 1
    ...
    def test_exception_during_gap_search_skipped(
        ...
        gaps = self.engine._find_gaps("machine learning and deep learning", DUMMY_VECTOR, ["ml-ai"])
        assert all(g.source == "gap" for g in gaps)
...
class TestFormatSuggestions:
    ...
    def test_with_related_and_gaps(self) -> None:
        result = SuggestionResult(
            query="transformers",
            related=[Suggestion("paper A", "related", 0.85, "ml-ai")],
            gaps=[Suggestion("attention heads", "gap", 0.2, "quant-trading")],
        )
...
class TestSuggest:
    ...
    def test_suggest_good_coverage(self, mock_emb_cls: MagicMock, mock_vs: MagicMock) -> None:
        ...
        result = engine.suggest("machine learning", collections=["ml-ai"])
    ...
    def test_suggest_poor_coverage_creates_gaps(
        ...
        result = engine.suggest("quantum computing and cryptography", collections=["ml-ai"])
    ...
    def test_suggest_max_suggestions_limit(
        ...
        result = engine.suggest("python asyncio", collections=["quant-trading", "ml-ai"])
